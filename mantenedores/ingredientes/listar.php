...
<div class="container mt-4">
    <?php if (isset($_GET['eliminado'])): ?>
        <div class='alert alert-success alert-dismissible fade show' role='alert'>
            Ingrediente ID <?php echo htmlspecialchars($_GET['id']); ?> eliminado exitosamente.
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="4" class="text-center">No se encontraron ingredientes.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
...
<script>
    ...
    eliminarModal.addEventListener('show.bs.modal', function (event) {
        ...
        var idIngrediente = button.getAttribute('data-id'); // Extrae el ID del ingrediente
        var modalForm = document.getElementById('eliminarForm');
        var inputIdIngrediente = modalForm.querySelector('#id_ingrediente');
        inputIdIngrediente.value = idIngrediente; // Coloca el ID en el input oculto del formulario
    });
</script>

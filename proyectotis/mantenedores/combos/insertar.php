...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        ...
        if (!empty($acompanamiento_id)) {
            $sql_acompanamiento = "INSERT INTO combo_acompanamiento (id_combo, id_acompanamiento) VALUES (?, ?)";
            $stmt_acompanamiento = $conexion->prepare($sql_acompanamiento);
            $stmt_acompanamiento->bind_param("ii", $id_combo, $acompanamiento_id);
            $stmt_acompanamiento->execute();
        }
        ...
        echo "<div class='alert alert-success' role='alert'>Combo agregado exitosamente.</div>";
    } else {
...
<div class="container mt-5">
    ...
    <form action="insertar.php" method="POST" class="mt-4">
        ...
        <div class="mb-3">
            <label for="acompanamiento_id" class="form-label">Seleccionar Acompañamiento</label>
            <select name="acompanamiento_id" id="acompanamiento_id" class="form-select">
                <option value="">Ninguno</option>
                <?php
                // Obtener acompañamientos
                $sql_acompanamientos = "SELECT * FROM acompanamiento";
                $result_acompanamientos = $conexion->query($sql_acompanamientos);
                while ($row_acompanamiento = $result_acompanamientos->fetch_assoc()) {
                    echo "<option value='" . $row_acompanamiento['id_acompanamiento'] . "'>" . $row_acompanamiento['nombre_acompanamiento'] . "</option>";
                }
                ?>
            </select>
        </div>

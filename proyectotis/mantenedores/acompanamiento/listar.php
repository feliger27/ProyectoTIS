        ...
        <tbody>
            <?php if ($result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?php echo $row['id_acompanamiento']; ?></td>
                        <td><?php echo $row['nombre_acompanamiento']; ?></td>
                        <td><?php echo $row['cantidad']; ?></td>
                        <td><?php echo $row['precio']; ?></td>
                        <td>
                            <a href="editar.php?id=<?php echo $row['id_acompanamiento']; ?>" class="btn btn-primary btn-sm">Editar</a>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="5" class="text-center">No se encontraron acompañamientos.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form id="eliminarForm" action="eliminar.php" method="POST">
                    <input type="hidden" name="id_acompanamiento" id="id_acompanamiento" value="">
...
<script>
    ...
    eliminarModal.addEventListener('show.bs.modal', function (event) {
        ...
        var idAcompanamiento = button.getAttribute('data-id'); // Extrae el ID del acompañamiento
        var modalForm = document.getElementById('eliminarForm');
        var inputIdAcompanamiento = modalForm.querySelector('#id_acompanamiento');
        inputIdAcompanamiento.value = idAcompanamiento; // Coloca el ID en el input oculto del formulario
    });
</script>
